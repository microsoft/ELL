...
namespace treeLayout
{
    ...
    void QpLayoutGenerator::BuildLayers(const ChildrenVectorType& Children)
    {
        ...
        std::vector<std::vector<size_t>> prev_layer_ancestors;
        BuildLayers(Children, 0, ancestors, prev_layer_ancestors);
    }

    template <typename ChildrenVectorType>
    void QpLayoutGenerator::BuildLayers(const ChildrenVectorType& Children, size_t index, std::vector<size_t>& ancestors, std::vector<std::vector<size_t>>& prev_layer_ancestors)
    {
        size_t depth = (size_t)ancestors.size();
        if (depth > (size_t)Children.size())
        {
        ...
        size_t closest_common_ancestor = 0;
        ...
        if (index < (size_t)Children.size())
        {
            ancestors.push_back(index);

            // recurse
            size_t child0 = Children[index].GetChild0();
            BuildLayers(Children, child0, ancestors, prev_layer_ancestors);

            size_t child1 = Children[index].GetChild1();
    ...
    template <typename ChildrenVectorType>
    std::vector<std::pair<double, double>> QpLayoutGenerator::SimpleLayout(const ChildrenVectorType& Children, size_t node_index, size_t depth)
    {
        ...
        // recurse
        size_t child0 = Children[node_index].GetChild0();
        size_t child1 = Children[node_index].GetChild1();
    ...
    template <typename ChildrenVectorType>
    void QpLayoutGenerator::IncrementOffsets(const ChildrenVectorType& Children, size_t node_index, double displacement)
    {
    ...
    {
        for (size_t t = 1; t <= _parameters.gdNumSteps; ++t)
        {
            ...
            double total_disp = 0;
            for (size_t index = 0; index < _offsets.size(); index++)
            {
    ...
    void QpLayoutGenerator::GdStep(const ChildrenVectorType& Children, double step_size)
    {
        ...
        for (size_t i = 0; i < (size_t)_gd_increment.size(); ++i)
        {
    ...
    void QpLayoutGenerator::ComputeGradient(const ChildrenVectorType& Children, const std::vector<double>& offsets, const std::vector<size_t>& depths, double step_size, std::vector<double>& grad)
    {
        ...
        for (size_t i = 0; i < Children.size(); ++i)
        {
            ...
            size_t child1 = Children[i].GetChild1();
    ...
    template <typename ChildrenVectorType>
    void QpLayoutGenerator::MoveParents(const ChildrenVectorType& Children, size_t node_index, double step_size)
    {
