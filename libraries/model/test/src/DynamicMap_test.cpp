...
{
    InputCallbackTester<double> stepDataCallbackTester;
    
    model::Model model;
    auto in = model.AddNode<model::InputNode<model::TimeTickType>>(2); // time signal
    auto source = model.AddNode<nodes::SourceNode<double>>(in->output, 3,
        [&stepDataCallbackTester](std::vector<double>& input) -> bool
        {
            return stepDataCallbackTester.InputCallback(input);
        }); // data signal
    ...
    auto dataSignal = std::vector<std::vector<double>>{ { 1.0, 2.0, 3.0 },
                                                        ...
                                                        { 7.0, 8.0, 9.0 },
                                                        { 10.0, 11.0, 12.0 } };
                                                        stepDataCallbackTester.Initialize(dataSignal);
