...
struct Emittable
{
    ...
    template <typename T>
    T GetDataAs() const { return reinterpret_cast<T>(_data); }

private:
    void* _data = nullptr;
};
static_assert(std::is_default_constructible_v<Emittable> &&
              std::is_nothrow_move_assignable_v<Emittable> &&
              std::is_nothrow_move_constructible_v<Emittable> &&
              std::is_swappable_v<Emittable>);
