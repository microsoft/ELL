...
public:
    ...
    void For(Value value, MemoryLayout layout, std::function<void(MemoryCoordinates)> fn);
...
private:
    ...
    virtual std::function<void(std::vector<Value>)> CreateFunctionImpl(std::string fnName, std::vector<Value> argValues, std::function<void(std::vector<Value>)> fn) = 0;
    virtual std::function<Value(std::vector<Value>)> CreateFunctionImpl(std::string fnName, Value returnValue, std::vector<Value> argValues, std::function<Value(std::vector<Value>)> fn) = 0;

    virtual Value StoreConstantDataImpl(ConstantData data) = 0;

    virtual void ForImpl(Value value, MemoryLayout layout, std::function<void(MemoryCoordinates)> fn) = 0;
