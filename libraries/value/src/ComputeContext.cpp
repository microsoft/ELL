...
Value ComputeContext::AllocateImpl(ValueType type, MemoryLayout layout)
{
    ...
    switch (type)
    {
    case ValueType::Boolean: return createValue(IdentityType<Boolean>{});
    case ValueType::Char8: return createValue(IdentityType<char>{});
    case ValueType::Byte: return createValue(IdentityType<uint8_t>{});
    case ValueType::Int16: return createValue(IdentityType<int16_t>{});
    case ValueType::Int32: return createValue(IdentityType<int32_t>{});
    case ValueType::Int64: return createValue(IdentityType<int64_t>{});
    case ValueType::Float: return createValue(IdentityType<float>{});
    case ValueType::Double: return createValue(IdentityType<double>{});
    default: throw LogicException(LogicExceptionErrors::notImplemented);
    }
}
...
Value ComputeContext::StoreConstantDataImpl(ConstantData data)
{
    Value value = std::visit(
        [](auto&& vector) -> Value
        {
            return Value(vector.data(), MemoryLayout({ (int)vector.size() }));
        }, data
    );
...
std::function<void(std::vector<Value>)> ComputeContext::CreateFunctionImpl(std::string fnName,
    std::vector<Value> expectedArgs, std::function<void(std::vector<Value>)> fn)
{
...
std::function<Value(std::vector<Value>)> ComputeContext::CreateFunctionImpl(std::string fnName, Value expectedReturn,
    std::vector<Value> expectedArgs, std::function<Value(std::vector<Value>)> fn)
{
    return [fn = std::move(fn), this, expectedReturn, expectedArgs = std::move(expectedArgs)](std::vector<Value> args)
    {
        assert(expectedArgs.size() == args.size());
        ConstantData movedOutOfScope;
        Value returnValue = expectedReturn;
            ...
            [&destination, &source](auto&& sourceData)
            {
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        auto sourceOffset = sourceLayout.GetLogicalEntryOffset(logicalCoordinates);
                        auto destinationOffset = destination.GetLayout().GetLogicalEntryOffset(logicalCoordinates);
...
void ComputeContext::ForImpl(Value value, MemoryLayout layout, std::function<void(MemoryCoordinates)> fn)
{
...
Value ComputeContext::BinaryOperationImpl(ValueBinaryOperation op, Value destination, Value source)
{
    ...
    std::visit(
        VariantVisitor {
            ...
            {
                using DestinationDataType = std::remove_pointer_t<std::decay_t<decltype(destinationData)>>;

                std::function<DestinationDataType(DestinationDataType, DestinationDataType)> opFn;
                switch (op)
                {
                ...
                {
                    auto numElements = destination.GetLayout().NumElements();
                    std::transform(destinationData, destinationData + numElements,
                                   sourceData,
                                   destinationData,
                                   opFn
                    );
                ...
                else
                {
                    ...
                    do
                    {
                        ...
                        auto sourceOffset = sourceLayout.GetLogicalEntryOffset(logicalCoordinates);
                        auto destinationOffset = destination.GetLayout().GetLogicalEntryOffset(logicalCoordinates);
                        *(destinationData + destinationOffset) = opFn(*(destinationData + destinationOffset), *(sourceData + sourceOffset));
                    }
                    while (IncrementMemoryCoordinate(coordinate, maxCoordinate));
                }
            }
