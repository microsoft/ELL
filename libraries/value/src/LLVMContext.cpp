...
std::pair<ValueType, int> LLVMTypeToVarType(llvm::Type* type)
{
    ...
    {
        case llvm::Type::TypeID::FloatTyID: return { ValueType::Float, 0 };
        case llvm::Type::TypeID::DoubleTyID: return { ValueType::Double, 0 };
        case llvm::Type::TypeID::IntegerTyID:
            switch (type->getIntegerBitWidth())
            {
                case 1: return { ValueType::Boolean, 0 };
                case 8: return { ValueType::Char8, 0 };
                case 16: return { ValueType::Int16, 0 };
                case 32: return { ValueType::Int32, 0 };
                case 64: return { ValueType::Int64, 0 };
                default: break;
            }
...
{
#define VALUE_TYPE_TO_VARIABLE_TYPE_MAPPING(x, y) case ValueType::x: return VariableType::y

#define VALUE_TYPE_TO_VARIABLE_TYPE_PTR(x) case ValueType::x: return VariableType::x ## Pointer
...
LLVMContext::LLVMContext(IRModuleEmitter& emitter) :
    _emitter(emitter), _computeContext(_emitter.GetModuleName())
{}

Value LLVMContext::AllocateImpl(ValueType type, MemoryLayout layout)
{
    return { Emittable{ _functionStack.top().get().Variable(ValueTypeToVariableType(type), layout.GetMemorySize()) }, layout };
}
...
std::function<void()> LLVMContext::CreateFunctionImpl(std::string fnName, std::function<void()> fn)
{
    ...
    return [fnName, this]
    {
        (void)_emitter.GetCurrentFunction().Call(fnName);
    };
}
...
std::function<Value()> LLVMContext::CreateFunctionImpl(std::string fnName, Value returnValue, std::function<Value()> fn)
{
    ...
    return [fnName, this]
    {
        return Emittable{_emitter.GetCurrentFunction().Call(fnName)};
    };
}

std::function<void(std::vector<Value>)> LLVMContext::CreateFunctionImpl(std::string fnName,
    std::vector<Value> argValues, std::function<void(std::vector<Value>)> fn)
{
    std::vector<VariableType> variableArgTypes(argValues.size());
    std::transform(argValues.begin(), argValues.end(), variableArgTypes.begin(),
    [](Value value)
    {
    ...
    {
        if (!std::equal(args.begin(), args.end(), argValues.begin(), argValues.end(),
            [](Value suppliedValue, Value fnValue) { return suppliedValue.GetBaseType() == fnValue.GetBaseType(); }))
        {
            throw InputException(InputExceptionErrors::invalidArgument);
        }

        std::vector<LLVMValue> llvmArgs(args.size());
        std::transform(args.begin(), args.end(), llvmArgs.begin(),
        [](Value& arg)
        {
...
std::function<Value(std::vector<Value>)> LLVMContext::CreateFunctionImpl(std::string fnName, Value returnValue,
    std::vector<Value> argValues, std::function<Value(std::vector<Value>)> fn)
{
    std::vector<VariableType> variableArgTypes(argValues.size());
    std::transform(argValues.begin(), argValues.end(), variableArgTypes.begin(),
    [](Value value)
...
void LLVMContext::ForImpl(Value value, MemoryLayout layout, std::function<void(MemoryCoordinates)> fn)
{
    if (!value.IsDefined())
    {
        throw InputException(InputExceptionErrors::invalidArgument);
    ...
    if (value.IsConstant())
    {
        _computeContext.For(value, layout, fn);
    }
    else
    {
...
void LLVMContext::CopyDataImpl(const Value& source, Value& destination)
{
    ...
    else
    {
                    ...
                    [destValue, &fn, &layout](auto&& data)
                    {
                        ...
                        do
                        {
                            fn.SetValueAt(
                                ...
                                fn.Literal(*(data + layout.GetEntryOffset(coordinate)))
                            );
                        }
                        while (IncrementMemoryCoordinate(coordinate, maxCoordinate));
                    }
        ...
        else
        {
            ...
            {
                _emitter.GetIREmitter().MemoryCopy(srcValue, destValue, fn.Literal(static_cast<int64_t>(layout.GetMemorySize())));
            }
            else
            {
                auto maxCoordinate = layout.GetActiveSize().ToVector();
        ...
        [&destination, op, &fn](auto&& sourceData)
        {
            ...
            else
            {
                ...
                case ValueBinaryOperation::divide:
                    opFn = [&fn, isFp](auto dst, auto src)
                    {
                        return fn.Operator(isFp ? TypedOperator::divideFloat : TypedOperator::divideSigned, dst, src);
                    };
                    break;
                }
                ...
                std::conditional_t<std::is_same_v<Emittable, SourceDataType>, LLVMValue, SourceDataType> srcValue = nullptr;
