...
Value::Value(Emittable data, std::optional<MemoryLayout> layout) :
    _data(data), _type(GetContext().GetType(data)), _layout(layout)
{}

Value::Value(ValueType type, std::optional<MemoryLayout> layout) :
    _type({ type, 0 }), _layout(layout)
{}
...
void Value::SetData(Value value)
{
    ...
    std::visit(
        VariantVisitor {
            [](Undefined) { throw InputException(InputExceptionErrors::invalidArgument); },
...
{
    return std::holds_alternative<Undefined>(_data);
}
...
{
    return _type.first == ValueType::Boolean && !IsPointer();
}

bool Value::IsInt16() const
{
    return _type.first == ValueType::Int16 && !IsPointer();
}

bool Value::IsInt32() const
{
    return _type.first == ValueType::Int32 && !IsPointer();
}

bool Value::IsInt64() const
{
    return _type.first == ValueType::Int64 && !IsPointer();
}
...
{
    return _type.first == ValueType::Float && !IsPointer();
}

bool Value::IsDouble() const
{
    return _type.first == ValueType::Double && !IsPointer();
}
...
bool Value::IsBooleanPointer() const
{
    return _type.first == ValueType::Boolean && IsPointer();
}

bool Value::IsShortPointer() const
{
    return _type.first == ValueType::Int16 && IsPointer();
}

bool Value::IsInt32Pointer() const
{
    return _type.first == ValueType::Int32 && IsPointer();
}

bool Value::IsInt64Pointer() const
{
    return _type.first == ValueType::Int64 && IsPointer();
}
...
bool Value::IsFloat32Pointer() const
{
    return _type.first == ValueType::Float && IsPointer();
}

bool Value::IsDoublePointer() const
{
    return _type.first == ValueType::Double && IsPointer();
}
...
{
    return Offset(static_cast<Value>(index));
}
