...
namespace nodes
{
    ...
    DelayNode<ValueType>::DelayNode(const model::PortElements<ValueType>& input, size_t windowSize) : Node({&_input}, {&_output}), _input(this, input, inputPortName), _output(this, outputPortName, _input.Size()), _windowSize(windowSize)
    {
    ...
    template <typename ValueType>
    void DelayNode<ValueType>::AddProperties(utilities::Archiver& description) const
    {
    ...
    template <typename ValueType>
    void DelayNode<ValueType>::SetObjectState(const utilities::Archiver& description, utilities::SerializationContext& context)
    {
