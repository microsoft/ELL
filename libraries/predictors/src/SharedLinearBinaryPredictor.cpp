...
namespace predictors
{
    ...
    {
        uint64 layerIndex = map.PushBack(std::make_unique<layers::Coordinatewise>(_sp_predictor->w, inputCoordinates, layers::Layer::Type::scale));
        auto coordinates = layers::GetCoordinateList(map, layerIndex);
        layerIndex = map.PushBack(std::make_unique<layers::Sum>(coordinates));

        map.PushBack(std::make_unique<layers::Coordinatewise>(_sp_predictor->b, layers::Coordinate{layerIndex, 0}, layers::Layer::Type::shift));
    }
}
