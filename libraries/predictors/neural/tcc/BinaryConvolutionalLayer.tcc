...
namespace neural
{
    ...
    BinaryConvolutionalLayer<ElementType>::BinaryConvolutionalLayer(const LayerParameters& layerParameters, const BinaryConvolutionalParameters& convolutionalParameters, ConstTensorReferenceType& weights) :
        ...
        _binarizedWeights(NumOutputChannels()),
        _binarizedShapedInput(NumOutputRowsMinusPadding() * NumOutputColumnsMinusPadding()),
        _filterMeans(NumOutputChannels()),
        _realValuedShapedInput(convolutionalParameters.receptiveField * convolutionalParameters.receptiveField * _layerParameters.input.NumChannels(), NumOutputRowsMinusPadding() * NumOutputColumnsMinusPadding()),
        _realValuedWeightsMatrix(NumOutputChannels(), convolutionalParameters.receptiveField * convolutionalParameters.receptiveField * _layerParameters.input.NumChannels()),
        _realValuedOutputMatrix(NumOutputChannels(), NumOutputRowsMinusPadding() * NumOutputColumnsMinusPadding())
    {
        if (weights.NumElements() != (NumOutputChannels() * _layerParameters.input.NumChannels() * convolutionalParameters.receptiveField * convolutionalParameters.receptiveField))
        {
    ...
    void BinaryConvolutionalLayer<ElementType>::ReceptiveFieldToBinaryRows(ConstTensorReferenceType input, std::vector<std::vector<uint64_t>>& shapedInput)
    {
        ...
        const size_t packedRowSize = (fieldVolumeSize + _binaryElementSize - 1) / _binaryElementSize;
        ...
        for (size_t outRow = 0; outRow < rowMax; outRow++)
        {
            ...
            for (size_t f = 0; f < fieldVolumeSize; f++)
            {
                ...
                const intptr_t sourcetDepth = volDepth;

                ElementType value = input(sourceRow, sourceCol, sourcetDepth);
