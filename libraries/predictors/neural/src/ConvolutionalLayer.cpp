...
namespace neural
{
    ConvolutionalLayer::ConvolutionalLayer(ConvolutionalParameters convolutionalParameters, const std::vector<double>& weights, const std::vector<double>& bias) :
        ...
        _fieldVolumeSize(convolutionalParameters.receptiveField * convolutionalParameters.receptiveField * convolutionalParameters.depth),
        _outputWidth(((convolutionalParameters.width - convolutionalParameters.receptiveField + 2 * convolutionalParameters.padding) / convolutionalParameters.stride) + 1),
        _outputHeight(((convolutionalParameters.height - convolutionalParameters.receptiveField + 2 * convolutionalParameters.padding) / convolutionalParameters.stride) + 1),
        _numNeuronsPerFilter(_outputWidth * _outputHeight),
        _weights(convolutionalParameters.numFilters, _fieldVolumeSize, weights),
        _bias(bias),
        _shapedInput(_fieldVolumeSize, _numNeuronsPerFilter),
        _output(convolutionalParameters.numFilters * _numNeuronsPerFilter),
        _outputMatrixReference(convolutionalParameters.numFilters, _numNeuronsPerFilter, _output.GetDataPointer())
    {
    ...
    ConvolutionalLayer::ConvolutionalLayer(ConvolutionalParameters convolutionalParameters, std::vector<double>&& weights, std::vector<double>&& bias) :
        ...
        _outputHeight(((convolutionalParameters.height - convolutionalParameters.receptiveField + 2 * convolutionalParameters.padding) / convolutionalParameters.stride) + 1),
        _numNeuronsPerFilter(_outputWidth * _outputHeight),
        _weights(convolutionalParameters.numFilters, _fieldVolumeSize, std::move(weights)),
        _bias(std::move(bias)),
        _shapedInput(_fieldVolumeSize, _numNeuronsPerFilter),
        _output(convolutionalParameters.numFilters * _numNeuronsPerFilter),
        _outputMatrixReference(convolutionalParameters.numFilters, _numNeuronsPerFilter, _output.GetDataPointer())
    ...
    void ConvolutionalLayer::ReadFromArchive(utilities::Unarchiver& archiver)
    {
        ...
        _outputMatrixReference.Swap(std::move(LayerReferenceMatrix(_convolutionalParameters.numFilters, _numNeuronsPerFilter, _output.GetDataPointer())));
    }
