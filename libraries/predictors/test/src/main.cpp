...
void ForestPredictorTest()
{
    ...
    // test Compute
    double output = forest.Predict(dataset::DoubleDataVector{ 0.2, 0.5, 0.0 }, tree0Root);
    testing::ProcessTest("Testing Compute(tree0)", testing::IsEqual(output, -3.0, 1.0e-8));

    output = forest.Predict(dataset::DoubleDataVector{ 0.2, 0.7, 0.0 }, tree0Root);
    testing::ProcessTest("Testing Compute(tree0)", testing::IsEqual(output, 1.0, 1.0e-8));

    output = forest.Predict(dataset::DoubleDataVector{ 0.5, 0.7, 0.7 }, tree0Root);
    testing::ProcessTest("Testing Compute(tree0)", testing::IsEqual(output, -3.0, 1.0e-8));

    output = forest.Predict(dataset::DoubleDataVector{ 0.5, 0.7, 1.0 }, tree0Root);
    testing::ProcessTest("Testing Compute(tree0)", testing::IsEqual(output, 5.0, 1.0e-8));

    output = forest.Predict(dataset::DoubleDataVector{ 0.2, 0.5, 0.0 });
    testing::ProcessTest("Testing Compute()", testing::IsEqual(output, -6.0, 1.0e-8));

    output = forest.Predict(dataset::DoubleDataVector{ 0.25, 0.7, 0.0 });
