...
class CntkLayersTestCase(unittest.TestCase):
    def test_dense_layer(self):
        ...
        weightTensor = cntk_to_ell.get_float_tensor_from_cnntk_dense_weight_parameter(denseLayer.parameters[0])
        ...
        orderedInputValues = cntk_to_ell.get_float_vector_from_cntk_array(inputValues)
        cntkResults = cntkModel(inputValues)
        orderedCntkResults = cntk_to_ell.get_float_vector_from_cntk_array(cntkResults)
        ellResults = predictor.Predict(orderedInputValues)

        # Compare the results
        np.testing.assert_array_equal(orderedCntkResults, ellResults, 'results for Dense layer do not match!')
    ...
    def test_max_pooling_layer(self):
        ...
        x = input((3,12,12)) # Input order for CNTK is channels, rows, columns
        ...
        layer = ELL.FloatPoolingLayer(layerParameters, poolingParameters, ELL.PoolingType.max)
        predictor = ELL.FloatNeuralNetworkPredictor([layer])

        # Get the results for both
        inputValues = np.arange(432, dtype=np.float32).reshape(3,12,12)
        cntkResults = cntkModel(inputValues)
        orderedCntkResults = cntk_to_ell.get_float_vector_from_cntk_array(cntkResults) # Note that cntk inserts an extra dimension of 1 in the front
        orderedInputValues = cntk_to_ell.get_float_vector_from_cntk_array(inputValues)
        ellResults = predictor.Predict(orderedInputValues)

        # Compare them
        np.testing.assert_array_equal(orderedCntkResults, ellResults, 'results for MaxPooling layer do not match!')
    ...
    def test_convolution_layer(self):
        ...
        convolutionLayer = Convolution((3,3), 5, pad=(True, True), strides=1, bias=False, init=0)
        x = input((2,3,4)) # Input order for CNTK is channels, rows, columns
        ...
        # create an ELL Tensor from the cntk weights, which re-orders the weights and produces an appropriately dimensioned tensor
        weightTensor = cntk_to_ell.get_float_tensor_from_cntk_convolutional_weight_parameter(convolutionLayer.parameters[0])
        ...
        layer = ELL.FloatConvolutionalLayer(layerParameters, convolutionalParameters, weightTensor)
        predictor = ELL.FloatNeuralNetworkPredictor([layer])
        ...
        # Compare the results
        np.testing.assert_array_equal(orderedCntkResults, ellResults, 'results for Convolution layer do not match!')
...
class CntkXorModelTestCase(unittest.TestCase):
    def test_simple_xor_model(self):
        ...
        self.assertAlmostEqual(result[0], 0, msg='incorrect prediction for [0, 0]')
        result = predictor.Predict([0, 1])
        self.assertAlmostEqual(result[0], 1, msg='incorrect prediction for [0, 1]')
        result = predictor.Predict([1, 0])
        self.assertAlmostEqual(result[0], 1, msg='incorrect prediction for [1, 0]')
        result = predictor.Predict([1, 1])
        self.assertAlmostEqual(result[0], 0, msg='incorrect prediction for [1, 1]')
        return
