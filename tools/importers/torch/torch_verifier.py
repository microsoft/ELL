...
class TorchModelVerifier:
    ...
    def __init__(self, torch_model, torch_model_source, input_shape, output_shape):
        ...
        _logger.info("Loading torch state from {}".format(torch_model_source))
    ...
    def load_image(self, filename):
        ...
        _logger.info("Loading image {} and reshaping to {}".format(filename, str(self.input_shape)))
    ...
    def export_to_onnx(self, model_path):
        _logger.info("Exporting model to ONNX: {}".format(model_path))
        tensor = self.input_tensors[0]
        self.torch_model.eval()
        self.torch_model(tensor)
        torch_out = torch.onnx._export(self.torch_model, tensor, model_path, export_params=True, verbose=True)
    ...
    def verify_ell_model(self, onnx_model_path, verify_compiled = True, arg_max_only = False):
        ...
        try:
            ...
            for test_input in self.input_tensors:
                ...
                if not arg_max_only:
                    ...
                    for key in info.layers.keys():
                        if input_index == 0:
                            _logger.info("----- Comparing Layer {} output -----".format(key))
                        ...
                        if node is not None:
                            ...
                            np.testing.assert_almost_equal(torch_output, ell_output, decimal=3, err_msg=(
                                    'results for ELL layer {} do not match torch output for row {}'.format(node.GetRuntimeTypeName(), input_index)))


                # compare whole model output but only the argmax of it, because sometimes model has Softmax but ELL does not.
                torch_prediction = np.argmax(torch_out)
                ell_prediction = np.argmax(ell_out)
                if verify_compiled: 
                    compiled_prediction = np.argmax(ell_out_compiled)
                    msg = "argmax of ELL result {}, ELL compiled result {} and argmax of torch output {} on row {}".format(ell_prediction, compiled_prediction, torch_prediction, input_index)
                else:
                    msg = "argmax of ELL result {} and argmax of torch output {} on row {}".format(ell_prediction, torch_prediction, input_index)
            ...
            _logger.error("Verification of model output failed: " + str(exception))
            raise
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("Verify the the torch model specified by the file 'torch_model_path'.\n" +
        "It will export the model to .onnx, convert it to .ell, and compile that .ell model\n" +
    ...
    parser.add_argument("model_source", help="path to the torch model_source state you want to load into your torch model")
    ...
    verifier = TorchModelVerifier.from_python_module(args.module, args.class_name, args.model_source, input_shape, output_shape)
