...
std::string LinearOperation::ToString(const std::string & sourceVariableName) const
{
    ...
    if (_b == 0)
    {
        if (_a != 1)
        {
            return utilities::Format::Printf("% * %", _a, sourceVariableName);
        }
        else
        {
    ...
    else
    {
        ...
        {
            return utilities::Format::Printf("% + %", sourceVariableName, _b);
        }
        else
        {
            return utilities::Format::Printf("% * % + %", _a, sourceVariableName, _b);
        }
    }
}
